...
public class BaseStartRed9Ball extends OpMode
{
    ...
    Pose start = new Pose(120, 127, 37);

    Pose motifPos = new Pose(88, 94, 90);

    Pose shoot = new Pose(88, 94, 45);

    Pose topIntakeStart = new Pose(102, 86, 0);

    Pose topIntakeEnd = new Pose(128, 86, 0);

    Pose midIntakeStart = new Pose(102,62,0);

    Pose midIntakeEnd = new Pose(125,62,0);

    Pose gatePos = new Pose(124,71,180);

    //Creating the Thread which sorts the balls
    public Thread OrderBalls;

    //Letting the robot know when sorting is still happening or if it is done
