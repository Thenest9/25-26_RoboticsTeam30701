...
public class BaseStartRed9Ball extends OpMode {
    ...
    private final Pose start = new Pose(120, 126, Math.toRadians(36));

    private final Pose motifPos = new Pose(91, 90, Math.toRadians(95));

    private final Pose shoot = new Pose(91, 90, Math.toRadians(45));

    private final Pose topIntakeStart = new Pose(102, 86, Math.toRadians(0));

    private final Pose topIntakeEnd = new Pose(127, 86, Math.toRadians(0));

    private final Pose midIntakeStart = new Pose(102, 62, Math.toRadians(0));

    private final Pose midIntakeEnd = new Pose(124, 62, Math.toRadians(0));

    private final Pose endPos = new Pose(125, 70, Math.toRadians(180));
